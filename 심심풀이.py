...
import time # 시간을 재기 위한 모듈
import win32api # 마우스 버튼 상태를 확인하기 위한 모듈
import pickle # 매크로 파일을 저장하기 위한 모듈
import os # 파일을 읽고 쓰기 위한 모듈
import queue # 큐를 사용하기 위한 모듈
# 큐가 뭔데?
# 큐는 FIFO(First In First Out) 방식으로 데이터를 저장하는 자료구조이다.
# 큐는 데이터를 넣는 쪽을 rear, 데이터를 꺼내는 쪽을 front라고 한다.

# 큐를 사용하는 이유는?
# 큐를 사용하는 이유는 매크로를 실행하는 동안에는 키보드와 마우스의 입력을 받지 않기 위함이다.
# 큐를 사용하지 않으면 매크로를 실행하는 동안에도 키보드와 마우스의 입력을 받기 때문에
# 매크로가 제대로 실행되지 않는다.
